...
class PriorityQueue(Container):
    ...
    def add(self, item):
        ...
        """
        i = 0
        # i stops incrementing when <timestamp> of <item> is no
        # longer larger than the <timestamp> of the item at index i
        while i != len(self._items) and item.__gt__(self._items[i]):
            i += 1
        # if the while loop iterated through the whole list
        if i == len(self._items):
            # the <timestamp> of <item> is the largest in the PriorityQueue
            # and <item> should be added to the end of it
            self._items.append(item)
        # if <item> and the item at index i share the same <timestamp>
        elif item.__eq__(self._items[i]):
            # add <item> after the item at index i
            self._items.insert(i + 1, item)
        # if the <timestamp> of <item> is smaller than the <timestamp> of
        # the item at index i
        else:
            # add <item> before the item at index i
